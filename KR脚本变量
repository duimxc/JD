##KR脚本变量
## 抽奖次数
export opencard_draw="10"
## 京喜助力码
export krjxcode="a3d80c0e0d0800250643572d934a5f7b"
## 极速版赚金币邀请码
export InviterPin="3YoXZ0v5JKz5WNO55-wQTg"
## 邀请领现金
export krlxjcode="3YoXZ0v5JKz5WNO55-wQTg"
## LZ店铺游戏 · 超级无线
## 1.是否做加购任务
export jd_wxgame_addCart="true"
## 关注店铺有礼
## 1.是否开卡
export jd_wxShopFollowActivity_openCard="true"
## 2.运行账号数量
export jd_wxShopFollowActivity_num="7"
## 分享有礼-加密
export jd_wxShareActivity_helpnum="40"
## 读秒拼手速通用活动
## 1.是否做加购任务
export jd_wxSecond_addCart="true"
## 生日礼包和会员等级礼包
## 1.运行账号数量，默认运行前7
export jd_wxMcLevelAndBirthGifts_num="7"
## 2.是否开卡，需要开卡变量值为 true，默认不开卡
export jd_wxMcLevelAndBirthGifts_openCard="true"
## 知识超人
## 1.是否开卡，默认不开卡
export jd_wxKnowledgeActivity_openCard="true"
## 2.运行账号数量，默认运行前7
export jd_wxKnowledgeActivity_num="7" 
## 加购有礼通用-加密
## 1.默认不开卡
export jd_wxCollectionActivity_openCard="true"
## 2.运行账号数量，默认前7账号  跑太多账号493！
export jd_wxCollectionActivity_num="7"
## LZ盖楼有礼
## 1.运行账号数量，默认运行前7
export jd_wxBuildActivity_num="7"
## 2.是否开卡，需要开卡变量值为 true，默认不开卡
export jd_wxBuildActivity_openCard="true"
## 微定制瓜分福袋-加密
## 1.做任务，点亮店铺
export jd_wdzfd_task="true"
## 特价版签到提现-加密
## 1.每次加速会扣除300金币
export JD_apSpeedUp="true"
## 超级无线店铺签到-监控版
## 1.运行账号数量
export COOKIE_NUM='7'
## JoinCommon金币卡开卡专用
## 1.开启开卡
export opencard_open="true"
## 2.开启加购
export opencard_addCart="true"
## 3.抽奖次数
export opencard_draw="3"
## 生鲜早起打卡(需要支付一元)
export morningScPins=""
## 赚钱大赢家提现
export isCashOut="true"
## 店铺抽奖通用活动-加密
## 1.是否开卡，默认不开卡
export LUCK_DRAW_OPENCARD="true"
## 2.是否推送通知，默认不推送 抽中才会推送，空气不推送
export LUCK_DRAW_NOTIFY="true"
## 3.运行账号数量，默认运行前7
export LUCK_DRAW_NUM="7"
## 4.账号间延迟，默认 3秒
export LUCK_DRAW_WAIT="3"
## 5.连续无次数跳出，默认 5 次，火爆账号请设置黑名单，否则也会占用次数
export LUCK_DRAW_Number="5"
## 汪汪乐园每日任务
export HELP_JOYPARK="YRbhz2jpdC0RAE0YJlu28w"
## 汪汪乐园合成
## 1.最大化硬币收益，如果合成后全部挖土后还有空位，则开启此模式（默认开启） 0关闭 1开启
export JOY_COIN_MAXIMIZE="1"
## JoyJd任务脚本
export comm_activityIDList="af2b3d56e22d43afa0c50622c45ca2a3"  
export comm_endTimeList="1639756800000"
export comm_tasknameList="京东工业品抽奖"
## LZ刮刮乐抽奖通用活动-加密
export jd_drawCenter_addCart="true"
## 完善信息有礼
## 1.运行账号数量，默认运行前7
export jd_completeInfoActivity_num="7"
## 2.是否开卡，需要开卡变量值为 true，默认不开卡
export jd_completeInfoActivity_openCard="true"
## 清空购物车-Sign版
JD_CART_REMOVE="true"

## 运行变量
## LZ组队瓜分京豆-加密
export jd_zdjr_activityId=""
export jd_zdjr_activityUrl=""
## LZ店铺游戏 · 超级无线
export jd_wxgame_activityId=""
## LZ让福袋飞
export jd_wxUnPackingActivity_activityId=""
## 店铺礼包特效
export jd_wxShopGift_activityUrl=""
## 关注店铺有礼
export jd_wxShopFollowActivity_activityUrl=""
## 分享有礼-加密
export jd_wxShareActivity_activityId=""
## 读秒拼手速通用活动
export jd_wxSecond_activityId=""
## 生日礼包和会员等级礼包
export jd_wxMcLevelAndBirthGifts_activityId=""
export jd_wxMcLevelAndBirthGifts_activityUrl="https://cjhydz-isv.isvjcloud.com"
## 知识超人
export jd_wxKnowledgeActivity_activityUrl=""
## 粉丝互动通用活动-加密
export jd_wxFansInterActionActivity_activityId=""
## 加购有礼通用-加密
export jd_wxCollectionActivity_activityUrl=""
## 集卡抽奖通用活动
export jd_wxCollectCard_activityId=""
## 购物车锦鲤通用活动
export jd_wxCartKoi_activityId=""
## LZ盖楼有礼
export jd_wxBuildActivity_activityId=""
## 微定制瓜分福袋-加密
export jd_wdzfd_activityId=""
export jd_wdzfd_activityUrl="https://cjhydz-isv.isvjcloud.com"
## 微定制瓜分京豆-加密
export jd_wdz_activityId=""
export jd_wdz_activityUrl="https://cjhydz-isv.isvjcloud.com"
## 收藏大师-幸运抽奖
export jd_lottery_activityUrl=""
## 收藏大师-关注商品
export jd_collect_shop_activityUrl=""
## 收藏大师-关注有礼
export jd_collect_item_activityUrl=""
## 收藏大师-加购有礼
export jd_cart_item_activityUrl=""
## 组队分豆-瓜分
export jd_teamAJ=""
export jd_teamFLP=""
## 飞利浦组队分豆-加密
export jd_teamFLP_activityId=""
## 店铺左侧刮刮乐
export jd_shopDraw_activityUrl=""
## 超级无线店铺签到-监控版
export jd_sevenDay_activityUrl=""
## 超级无线店铺签到
export LZKJ_SEVENDAY=""
export LZKJ_SIGN=""
export CJHY_SEVENDAY=""
export CJHY_SIGN=""
## JoinCommon金币卡开卡专用
export opencard_id=""
## 盲盒任务抽京豆
export jd_mhurlList=""
## 邀请入会有礼（lzkj_loreal）
export jd_lzkj_loreal_invite_url=""
## 关注有礼（lzkj_loreal）
export jd_lzkj_loreal_followShop_url=""
## 幸运抽奖（lzkj_loreal）
export jd_lzkj_loreal_draw_url=""
## 加购有礼（lzkj_loreal）
export jd_lzkj_loreal_cart_url=""
## 店铺抽奖通用活动-加密
export LUCK_DRAW_URL=""
## 京东抽奖机加购-KR
export JD_Lottery_cart=""
## 京东抽奖机通用-KR
export JD_Lottery=""
## 邀请入会赢好礼（京耕）
export jd_showInviteJoin_activityUrl=""
## LZ刮刮乐抽奖通用活动-加密
export jd_drawCenter_activityId=""
## LZ每日抢好礼通用活动
export jd_daily_activityId=""
## 完善信息有礼
export jd_completeInfoActivity_activityId=""
export jd_completeInfoActivity_activityUrl="https://cjhydz-isv.isvjcloud.com"
export jd_completeInfoActivity_venderId=""
## CJ组队瓜分京豆-加密
export jd_cjhy_activityId=""
export jd_cjhy_activityUrl="https://cjhydz-isv.isvjcloud.com"
## CJ每日抢好礼通用活动
export jd_cjdaily_activityId=""
## 品类联合抽奖
export jd_categoryUnion_activityId=""
## 入会开卡领取礼包
export VENDER_ID=""
OPENCARD_QZ="true"